...
import WinTable from "../requests/winner";
...
export class ViewHtml {
  ...
  public createMainPage() {
    ...
    const changeContent: HTMLElement = document.createElement('div');
    ...
    const totalCar: HTMLElement = document.createElement('div');
    totalCar.classList.add('totalCar');

    const totalCountWin: HTMLElement = document.createElement('div');
    totalCountWin.classList.add('totalWin');

    const containerCar: HTMLElement = document.createElement('div');
  ...
  public createHeader(): void {
    ...
    const buttonGaragePage: HTMLButtonElement = document.createElement('button');
    buttonGaragePage.classList.add('header__button');
    buttonGaragePage.textContent = 'Garage';

    const buttonWinnerPage: HTMLButtonElement = document.createElement('button');
  ...
  public generateCar(): void {
    const containerCar: HTMLDivElement = document.createElement('div');
    containerCar.classList.add('conteiner__create-car');
    this.containerCar = containerCar

    const containerAddCar: HTMLElement = document.createElement('div');
    containerAddCar.classList.add('conteiner__add-car');

    const inputCarCreate: HTMLInputElement = document.createElement('input');
    inputCarCreate.classList.add('input__create-car');

    //saved in ls
    const savedInputValue: string | null = localStorage.getItem('inputCarValue');
    if (savedInputValue) {
    ...
    //saved in ls
    const savedColorInput: string | null = localStorage.getItem('colorInputValue');
    if (savedColorInput) {
    ...
    const namePage: HTMLElement = document.createElement('h1');
  ...
  public changeCar(): void {
    const containerChangeCar: HTMLDivElement = document.createElement('div');
    containerChangeCar.classList.add('conteiner__change-car');
    this.containerChangeCar = containerChangeCar;

    const inputCarChange: HTMLInputElement = document.createElement('input');
    inputCarChange.classList.add('input__change-car');


    //saved in ls
    const savedInputValueChange: string | null = localStorage.getItem('inputCarValueChange');
    ...
    const colorInputChange: HTMLInputElement = document.createElement('input');
    colorInputChange.classList.add('color__change-car')
    colorInputChange.setAttribute('type', 'color');

    //saved in ls
    const savedInputColorChange: string | null = localStorage.getItem('inputCarColorChange');
    ...
    const buttonChangeCar: HTMLButtonElement = document.createElement('button');
    ...
    try {
      ...
      if (Array.isArray(cars)) {
        ...
        cars.forEach((car: Car) => {
          const lot: HTMLDivElement = document.createElement('div');
          lot.classList.add('car');

          const idCar: HTMLElement = document.createElement('div');
          idCar.textContent = `ID: ${car.id}`;
          idCar.classList.add('text')

          const nameCar: HTMLElement = document.createElement('div');
          nameCar.textContent = `Name: ${car.name}`;
          nameCar.classList.add('text')

          const carDistance: HTMLElement = document.createElement('div');
          const distanceDiv = document.createElement('div');
          distanceDiv.classList.add('distance');
          distanceDiv.id = `distance_${car.id}`;

          const carDiv: HTMLElement = document.createElement('div');
          carDiv.classList.add('car_image');
          carDiv.id = `car_${car.id}`;

          const svgString: string = `<svg xmlns="http://www.w3.org/2000/svg" id="carSvg" width="120" height="120" fill="${car.color}" viewBox="0 0 256 256" xml:space="preserve">
        ...
        <path d="m225.53 116.243-41.322-10.597-19.695-11.428a100.079 100.079 0 0 0-50.164-13.502H84.625c-19.176 0-37.494 7.168-51.578 20.184a10.864 10.864 0 0 1-7.401 2.895h-6.29c-7.637 0-14.086 5.718-15.002 13.302l-2.846 23.64a13.903 13.903 0 0 0 9.09 14.787l12.923 4.693a29.276 29.276 0 0 1-1.416-8.978c0-16.197 13.176-29.373 29.373-29.373s29.373 13.176 29.373 29.373c0 4.442-1.02 8.64-2.793 12.42h107.797c-1.773-3.78-2.793-7.978-2.793-12.42 0-16.197 13.176-29.373 29.373-29.373s29.373 13.176 29.373 29.373c0 3.875-.773 7.567-2.142 10.956l8.192-1.31a7.621 7.621 0 0 0 6.449-7.559c0-17.486-11.836-32.736-28.778-37.083zM77.1 111.457c-3.673 0-7.003-1.714-9.136-4.7-2.135-2.988-2.678-6.694-1.486-10.167l1.843-5.384c5.291-1.313 10.748-2.057 16.307-2.057h6.46l9.458 22.308H77.1zm82.673 0H109.7l-9.458-22.308h14.106a91.628 91.628 0 0 1 45.932 12.361l12.876 7.469c-4.288 1.613-8.77 2.478-13.384 2.478z"/>
          ...
          //button
          const containerBtn: HTMLElement = document.createElement('div');
          ...
          const buttonDeleteCars: HTMLButtonElement = document.createElement('button');
  ...
  public generateButtonFunctional(): void {
    const containerButtonFunctional: HTMLDivElement = document.createElement('div');
    ...
    const buttonGenerateCars: HTMLButtonElement= document.createElement('button');
  ...
  public async paginationWin() {
    ...
    const savedPageNumberWin: string | null = localStorage.getItem('pageNumberWin');
    if (savedPageNumberWin) {
      this.pageWin = parseInt(savedPageNumberWin);
    } else {
      this.pageWin = 1;
    }
    const buttonPreviousWin: HTMLButtonElement = document.createElement('button');
    ...
    const buttonNextWin: HTMLButtonElement = document.createElement('button');
